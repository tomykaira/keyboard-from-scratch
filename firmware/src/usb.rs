...
use crate::stm32f103::USB as USBRegs;
use crate::{descr, pma};
use core::cmp;
use stm32f1::stm32f103;
use vcell::VolatileCell;

pub static DEVICE_DESCR: descr::DeviceDescriptor = descr::DeviceDescriptor {
...
pub static CONFIG_DESCR: CompositeConfigDescriptor = CompositeConfigDescriptor {
...
static STRINGS: &[&str] = &["tomykaira", "FLAT6", "1"];
...
impl<'a> USBKbd<'a> {
    ...
    pub(crate) fn hid_send_keys(&mut self, keys: &[u8]) -> Option<()> {
        self.ep_write_packet(EPAddr::new(0x81), keys)
    }

    pub(crate) fn usb_poll(&mut self) {
